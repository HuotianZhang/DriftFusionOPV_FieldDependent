function y=EquilibratePNHCT_forMarcus(Vapp,p)

% Swicth off mobility and set time to short time step
%p=p1; p is DP, deviceparams
% p.discretetrap=0;
% EAN0=p.EAn;
%p.NI = a;
...
p.Experiment_prop.BC=0; %boundary condition case 0-4
...
tic
disp('first solution')
sol = pndriftHCT_forMarcus(sol,p);
toc
tic
p.Experiment_prop.equilibrium=0;
...
p.Time_properties.t0 = p.Time_properties.tmax/1e3;
sol_eq = pndriftHCT_forMarcus(sol, p);
p=UpdateLayers(p);
sol_eq = pndriftHCT_forMarcus(sol_eq, p);
y=sol_eq;
toc
